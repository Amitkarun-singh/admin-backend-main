...
const bulkStudentUpload = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    for (const [index, row] of records.entries()) {
        ...
        if (classRecord) {
          ...
          if (row.section_name) {
            const sectionRecord = await AdminSection.findOne({
              where: { class_id: resolvedClassId, section_name: row.section_name },
              transaction,
            });
      ...
      await ParentStudentMap.create({ parent_id: parent.parent_id, student_id: student.student_id }, { transaction });
    ...
    await AdminSchool.increment("student_count", { by: createdCount, where: { school_id }, transaction });
    await transaction.commit();
    fs.unlinkSync(file.path);

    return res.status(201).json(new ApiResponse(201, { created: createdCount }, "Students uploaded successfully"));
  } catch (error) {
    await transaction.rollback();
